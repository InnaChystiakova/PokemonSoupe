    ...
    func jsonData(fileName: String) throws -> Data {
        guard let fileURL = Bundle(for: type(of: self)).url(forResource: fileName, withExtension: "json") else {
            throw NSError(
                domain: "MissingFile",
                code: 404,
                userInfo: [NSLocalizedDescriptionKey: "Missing file: {\(fileName)}.json"]
            )
        }
        ...
        do {
            ...
            throw NSError(
                ...
                code: 500,
                userInfo: [NSLocalizedDescriptionKey: "Failed to read JSON from file: \(error.localizedDescription)"]
            )
        }
    }
}
